...
def capture(unit_number, module_number, port_number):
    ...
    return messages
def stop_all(unit_number, module_number, port_number, when=None, time=None):
    stop = []
    ...
    return stop
def stop_capture(unit_number, module_number, port_number, when=None, time=None):
    stop = []
    stop.append(':UENTry:ID ' + str(unit_number) + '\n')
    stop.append(':MODule:ID ' + str(module_number) + '\n')
    stop.append(':PORT:ID ' + str(port_number) + '\n')
    stop.append(':CAPTure:STOP\n')
    return stop
def stop_counter(unit_number, module_number, port_number, when=None, time=None):
    stop = []
    stop.append(':UENTry:ID ' + str(unit_number) + ';')
    stop.append(':MODule:ID ' + str(module_number) + ';')
    stop.append(':PORT:ID ' + str(port_number) + ';')
    stop.append(':COUNter:STOP;')
    return stop
def stop_stream(unit_number, module_number, port_number, when=None, time=None):
    stop = []
    stop.append(':UENTry:ID ' + str(unit_number) + '\n')
    stop.append(':MODule:ID ' + str(module_number) + '\n')
    stop.append(':PORT:ID ' + str(port_number) + '\n')
